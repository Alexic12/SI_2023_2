...
class Neural:
    ...
    def run_model(self,model,file_name,iter,alfa,test_split):
        ...
        Train_features, test_features, train_label, test_labels = data.data_process(file_name,test_split)
        if model == 'perceptron':
            print('Running Perceptron Model')
            #Code for the perceptron model no vamos a usar libreriar.
            P = Perceptron()
            P.run(Train_features, test_features, train_label, test_labels,iter,alfa)
        elif model == 'ffm':
