    ...
    def run_model(self, model, file_name, iter, alfa, test_split, norm, stop_condition, neurons, avoid_col, chk_name, train, data_type):
        ...
        if model == 'conv_tf':
            ...
            if data_type == 'time_series':
                window_size =  2 # cambiar el dato aqui para quitar los que no pesa dos por cada uno. 
                horizon_size = 1
                train_features, test_features, train_labels, test_labels, original_features, original_labels = data.timeseries_process_adapt(window_size, horizon_size, file_name, test_split, norm) # se cambia a adapt cuando tengo pid  y son 4 ventanas 
