...
class Data:
    ...
    def timeseries_process(self, window_size, horizon_size, file, test_split, norm):
        ...
        for i in range(data_length - window_size - horizon_size):
            #vector =np.concatenate((array_raw[1,i:i+window_size+horizon_size],array_raw[0, i:i+window_size+horizon_size]))  ## Directa POSICION AL FINAL 
            vector =np.concatenate((array_raw[0,i:i+window_size+horizon_size],array_raw[1, i:i+window_size+horizon_size])) ## Inversa ENTRADA AL FINAL
            #por que el uno es el miu. 
        ...
        excel_filename = 'DATA_INVR_ORGANIZADA.xlsx' # EL imprime esto aqui 
    ...
    def timeseries_process_adapt(self, window_size, horizon_size, file, test_split, norm):
        ...
        for i in range(data_length - window_size - horizon_size):
            ...
            #cero es la posicion, tres es el error, 2 es el setpoint , 1 es el miu. 1 es directa miu. 
            time_series_arr[i] = vector
