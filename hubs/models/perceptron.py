...
class Perceptron:
    ...
    def run(self, train_features, test_features, train_labels, test_labels, iter, alfa):
        ...
        Yd = np.zeros((train_labels.shape[1],1)) #Feature vector

        Ek = np.zeros((train_labels.shape[1],1)) #Error vector

        ecm = np.zeros((train_labels.shape[1],1)) #ECM vector for each items
        ...
        for it in range(0,iter):
            for d in range(0, train_features.shape[0]):
                ...
                #Lets calculate error for each neuron

                #Pass train_label to Yd vector
