...
class xgb:
    ...
    def run(self, train_features, test_features, train_labels, test_labels, original_features, original_labels, iter, alfa, stop_condition, chk_name, train, neurons):
        ...
        ## se recommienda una neurona extra a la cantidad de entradas y para saber cuantas entradas va a tener el modelo en total, se multiplica por la profundidad
        model = self.build_model((train_features.shape[1]+1)*self.depth, alfa, 1)

        ##lets create an evaluation set
        eval_set = [(train_features, train_labels),(test_features, test_labels)] ## Lista de tuplas, dos tuplas 
                                                                            ## la primera con los train, la otra
                                                                            ## con los test

        if train:
            ##lets train the model
            model.fit(train_features, train_labels, eval_metric='mae', eval_set=eval_set, verbose=True) ## mae es 
                                                                                                ## mean average error
                                                                                                ## parecido a mse
    ...
    def build_model(self, n_estimators, learning_rate, verbosity):
        model = xg.XGBRegressor(
            objective='reg:squarederror', ##Loss funcion for training determination, con esta se determina si esta aprendiendo o no
            colsample_bytree=0.5, ##Entre cero y uno, evita el sobreajuste, en este caso va a tomar el 50% de los datos al azar, en cada creacioin de un nuevo arbol no va a tomar todas las caracteristicas, sino que va a tomar el 50% al azar, un arbol distinto cada vez que se corre
            learning_rate=learning_rate, #tasa de aprendizaje
            n_estimators=n_estimators, ##number of ramifications or branches (como si fueran neuronas)
            ...
            gamma=0, ## Me dice cuantos cortes son ramdom (cero es totalmente random) reduces random value for reg_lambda cuts (0-1)
            ...
            seed=20, ##Puede ser cualquier cosa, es el valor inicial Seed for random value, for reproductibility 
            tree_method='hist', ##ramification methos (Hist: reduces significantly the amount of data to be processed)
            updater='grow_quantile_histmaker,prune' ## crea las ramas de todos los valores de salida, simplemente entrena de atras hacia adelante
        )
