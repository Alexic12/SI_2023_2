...
class PerceptronMulti:
    ...
    def run(self, train_features, test_features, train_labels, test_labels, iter, alfa, stop_condition):
        ...
        Cjk = np.zeros((train_labels.shape[1], Hj.shape[0])) # Weights for the output layer

        Ak = np.zeros((train_labels.shape[1],1)) # Agregation function for the output layer

        Yk = np.zeros((train_labels.shape[1],1)) # Activation function for the output layer
        ...
        for it in range(0, iter):
            for d in range(0, train_features.shape[0]):
                ...
                ##lets calculate the agregation function for each output layer
                for n in range(0, train_labels.shape[1]):
                    for n_input in range(0,Hj.shape[0]):
                        Ak[n][0] = Ak[n][0] + Hj[n_input][0]*Cjk[n][n_input]


                ##lets calculate the activation function of the ouput layer
                for n in range(0, train_labels.shape[1]):
                    Yk[n][0] = 1/(1+np.exp(-Ak[n][0]))


                ##Lets fill the Yd vector with the labels for this data point
                ...
                ##Now we train Wij
                for h in range(0, Hj.shape[0]-1): ##or hidden_neurons
                    for i in range(0, Xi.shape[0]):
                        for o in range(0, Yk.shape[0]):
                            Wij[h][i] = Wij[h][i] + alfa*Ek[o][0]*Yk[o][0]*(1-Yk[o][0])*Cjk[o][h+1]*Hj[h+1][0]*(1-Hj[h+1][0])*Xi[i][0]
            ...
            ##Lets store the total ecm forthat specific iteration
            for n in range(0, Yk.shape[0]):
                ecmT[n][it] = ecm[n][0]
                ecm[n][0] = 0
