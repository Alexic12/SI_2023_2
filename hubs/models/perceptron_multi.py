...
class PerceptronMulti:
    ...
    def run (self, train_features, test_features, train_labels, test_labels,iter,alfa, stop_condition):
        ...
        #arbitratry value, not absolute
        hidden_neurons = train_features.shape[1]+ 1

        Xi=np.zeros((train_features.shape[1],1)) #input vecotr 

        Wij= np.zeros((hidden_neurons,train_features.shape[1])) # wieght matrix

        Aj= np.zeros((hidden_neurons,1)) # Agregation vector for the hidden layer 

        Hj=  np.zeros((hidden_neurons + 1,1)) #activade function for hidden layer

        Cjk=  np.zeros((train_labels.shape[1],Hj.shape[0])) # wieght for outputs layer
        ...
        ##Fill the weight Wij matrix before trainning 
        ...
        ##Fill the weight Cjk matrix before trainning 
        ...
        for it in range(0,iter):
            for d in range (0,train_features.shape[0]):
                ...
                ##Lets calculate the error fot this data point
                ...
                ## Because the back propagation we first train Cjk
                ...
                ##Lets train de Wij weights
                for h in range(0,Hj.shape[0]-1):  # or use hidden_neurons
