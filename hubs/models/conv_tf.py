...
##capas neuronales keras tiene todo internamente
...
class conv_tf:
    ...
    def run(self, train_images, test_images, train_labels, test_labels, iter):
        print (train_images)#60000 images 28x28 y viene en escala de grises
        print (train_images.shape[0])
        print (train_images.shape[1])

        ##Reshape images to specify that its a single channel of color. viene el rgb y es lo en escala grises
        train_images= train_images.reshape(train_images.shape[0], 28, 28, 1)
        test_images= train_images.reshape(test_images.shape[0], 28, 28, 1)
        ##se pone 28 porque es el tamaño de las imagenes, la base de datos viene por defecto en 28
        ##pero si hubiera otra base de datos dejarlo por defecto 28

        ##Lets normalize the image, se / por 255 porque es el tamaño mas grande que puede llegar a tener 
        ## las imagenes siempre
        train_images = train_images /255.0
        test_images = test_images /255.0

        ##Lets graph some images for example
        ...
        ##Lets show the accuracy of the model
