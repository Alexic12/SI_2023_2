...
class MassDamper:
    def __init__(self):
        ...
        xg = xgb(10) # Debe tener la misma profunididad que el modelo que se guardo
        self.model = xg.load_model(name = 'IDENT_FINAL_PID',inputs =  7, alfa = 0.02)
    ...
    def system_equations(self, t, y):
        ...
        dtheta_dt = omega
        domega_dt = -(g/l)*theta - (b/m)*omega + ((self.U*10)/(m*l**2))
    ...
    def run_simulation(self):
        ...
        plt.xlabel('Time (ms)')
        plt.ylabel('Value')
        plt.title('Pendlum under gravity')
        plt.grid(True)
        ##Itialize lines for position, velocity and U
        position_line, = plt.plot([],[],label='Theta (X)')
        velocity_line, = plt.plot([],[],label='Omega (V)')
        ...
        excel_filename = 'Toma_datos_planta_finalPID.xlsx'
